...
export const comparisonInput = z.object({
    ...
    const seen = new Set();
    rows.forEach((r, i) => {
        if (seen.has(r.shipment_id))
            ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["rows", i, "shipment_id"], message: "Duplicate shipment ID: compare one selected candidate per shipment to avoid double-counting" });
        seen.add(r.shipment_id);
    });
});
export function compareFreightCosts(input) {
...
export function registerWorkflows(server) {
    ...
    server.registerTool("compare_freight_costs", {
        ...
        try {
            return { content: [{ type: "text", text: JSON.stringify(compareFreightCosts(input), null, 2) }] };
        }
        catch (error) {
            return { isError: true, content: [{ type: "text", text: error instanceof z.ZodError ? error.message : "Could not compare supplied freight costs." }] };
        }
    });
}
