...
export class WarpClient {
    ...
    async _selfServeQuote(mode, params) {
        ...
        const headers = { "user-agent": USER_AGENT, ...this.extraHeaders(), "Content-Type": "application/json" };
        if (key)
    ...
    async _selfServe(method, path, opts) {
        const key = this.getApiKey();
    ...
    async book(params) {
        ...
        if (params.accessorials)
            body.accessorials = params.accessorials;
        if (params.pickup_window)
