...
export function registerTools(server, client, getApiKey) {
    ...
    server.tool("warp_book", "Book a quoted shipment using any quote_id or option id returned from a quote tool (Warp or market carrier). Requires quote_id + pickup and delivery addresses. Auth required.", {
        ...
        const start = Date.now();
        try {
            // Charge the saved Stripe card before booking
            const quoteId = params.quote_id;
            const cachedAmount = quoteAmountCache.get(quoteId);
            if (!cachedAmount) {
                return { content: [{ type: "text", text: `Cannot book: no cached quote amount for ${quoteId}. Please run warp_ltl_quote (or van/box-truck/ftl quote) first, then book immediately after.` }], isError: true };
            }
            ...
            if (!chargeRes.ok || chargeBody.status !== "succeeded") {
                ...
                if (chargeRes.status === 402 || String(msg).toLowerCase().includes("card") || String(msg).toLowerCase().includes("payment")) {
                    return { content: [{ type: "text", text: `No payment method on file. Add a card at https://wearewarp.com/agents/onboard then try again.` }], isError: true };
                }
                return { content: [{ type: "text", text: `Payment failed: ${msg}` }], isError: true };
            }
            const body = {
    ...
    // ── 7. warp_cancel ──────────────────────────────────────────────
    server.tool("warp_cancel", "Attempt to cancel a booking. Auth required. NOTE: Warp's API does not allow customer self-cancellation — this will return an error. Cancellations must be requested via Warp support. Still try this tool first so the user gets the accurate error message.", {
    ...
    // ── 10. warp_rate_card ──────────────────────────────────────────
    server.tool("warp_rate_card", "Get your negotiated rate card. Auth required.", {}, async () => {
        const start = Date.now();
        try {
    ...
    server.tool("warp_get_documents", "List shipment documents (BOL, POD, customs forms, etc.). Returns URLs or document references. Auth required.", {
        order_id: z.string().describe("Order ID (typically the same as shipment_id)"),
    }, async (params) => {
        const start = Date.now();
        try {
    ...
    server.tool("warp_multistop_quote", "Quote an FTL multi-stop shipment (one truck, multiple pickups/deliveries). Pass pickup_date plus an ordered list of stops (each with type=pickup or delivery and address). Auth required.", {
        pickup_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).refine((d) => validateDate(d) === true, (d) => ({ message: validateDate(d) })).describe("First pickup date YYYY-MM-DD"),
    ...
    server.tool("warp_multistop_book", "Book a quoted multi-stop FTL shipment. Pass the quote_id from warp_multistop_quote plus the same stops array with full address/contact info on each stop. Auth required.", {
        ...
        try {
            ...
            trackEvent({
                ...
                success: true,
                tracking_number: bookData?.trackingNumber,
                order_id: bookData?.orderId,
                quote_id: params.quote_id,
                amount_usd: bookData?.totalPrice,
                mode: 'ftl',
                duration_ms: Date.now() - start,
            });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
