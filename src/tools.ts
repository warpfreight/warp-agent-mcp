...
function summarizeBookingRows(rows: Record<string, unknown>[]): {
...
export function registerTools(server: McpServer, client: WarpClient, getApiKey: () => string | undefined) {
  ...
  // Register each tool with a top-level display `title` so MCP clients that render
  // "<server> <tool>" don't show a redundant "warp warp_…". Clients display the
  ...
  function tool<Args extends z.ZodRawShape>(
    ...
    description: string,
    schema: Args,
    annotationsOrHandler: ToolAnnotations | ToolCallback<Args>,
    maybeHandler?: ToolCallback<Args>,
  ): RegisteredTool {
    ...
    const existingAnn = hasAnnotations ? (annotationsOrHandler as ToolAnnotations) : undefined;
    const handler = (hasAnnotations ? maybeHandler : annotationsOrHandler) as ToolCallback<Args>;
    const title = existingAnn?.title ?? deprefixed;
    const annotations: ToolAnnotations = { ...existingAnn, title };
  ...
  const vanQuoteTool = tool(
    ...
    async (params) => {
      ...
      try {
        ...
        if (prohibitedIssue) return { content: [{ type: "text", text: prohibitedIssue }], isError: true };
    ...
    async (params) => {
      ...
      try {
        ...
        const opts = Array.isArray(marketOptions)
          ? (marketOptions as Array<Record<string, unknown>>)
          : [];
  ...
  type PricedMode = {
    ...
    requested_mode?: QuoteMode;
    mode_substituted?: Record<string, unknown>;
    ...
    async (params) => {
      ...
      try {
        ...
        for (const row of results) {
          ...
          const substitution = det.mode_substituted as Record<string, unknown> | undefined;
        ...
        const competitors = (marketRows as Array<Record<string, unknown>>)
          .filter((o) => o && o.is_warp !== true && typeof o.price_usd === "number");
        let market_benchmark: Record<string, unknown> | null = null;
        if (wantBenchmark && competitors.length) {
          const prices = competitors.map((o) => o.price_usd as number).sort((a, b) => a - b);
    ...
    async (params) => {
      ...
      try {
        ...
        for (let i = 0; i < rawLanes.length; i++) {
          ...
          const prohibitedIssue = checkProhibited(lane.commodity as string | undefined);
          if (prohibitedIssue) return { content: [{ type: "text", text: `${laneLabel}, commodity "${String(lane.commodity)}": ${prohibitedIssue}` }], isError: true };
          const commodityIssue = checkCommodity(lane.commodity as string | undefined);
          if (commodityIssue) return { content: [{ type: "text", text: `${laneLabel}, commodity "${String(lane.commodity)}": ${commodityIssue}` }], isError: true };
          ...
          const accIssue = checkAccessorials(
            lane.pickup_services as string[] | undefined,
            lane.delivery_services as string[] | undefined,
          );
          if (accIssue) return { content: [{ type: "text", text: `${laneLabel}: ${accIssue}` }], isError: true };
        }
  ...
  tool(
    ...
    async (params) => {
      ...
      try {
        ...
        const bookedPrice =
          (typeof data.price_usd === "number" && data.price_usd) ||
  ...
  const batchBookTool = tool(
    ...
    async (params) => {
      ...
      try {
        ...
        const intl = rows
          .map((r, i) => {
            const pickup   = (r.pickup   ?? sharedPickup)   as Record<string, unknown> | undefined;
            const delivery = (r.delivery ?? sharedDelivery) as Record<string, unknown> | undefined;
  ...
  tool(
    ...
    async (params) => {
      ...
      try {
        ...
        type MultistopStopInput = { stop_index: number; address: { zipcode: string } };
        const legs = params.shipments as Array<{ pickup_info: MultistopStopInput; delivery_info: MultistopStopInput }>;
        ...
        for (let li = 0; li < legs.length; li++) {
          const leg = legs[li];
          for (const side of ["pickup_info", "delivery_info"] as const) {
            const info = leg[side];
  ...
  const listBookingsTool = tool(
    ...
    async (params) => {
      ...
      try {
        ...
        return bookingsToolResult(data as Record<string, unknown>);
      } catch (err) {
        trackEvent({
          product: 'warp-agent',
          source: 'mcp',
  ...
  tool(
    ...
    async (params) => {
      ...
      try {
        ...
        const data = await client.proposeAutomation(body) as Record<string, unknown>;
        ...
        return { content: [{ type: "text", text:
          `Automation PROPOSED — not yet active. The account owner has been emailed an approval link; nothing books until they approve, and Warp never activates an automation from chat.\n\n` +
  ...
  tool(
    ...
    { title: "Automation Authorization Record", readOnlyHint: true },
    async (params) => {
      const start = Date.now();
      try {
    ...
    async (params) => {
      ...
      try {
        ...
        try {
          ...
          if (res.ok) rows = pickRows(await res.json());
        } catch {
        ...
        summary.scope = scope;
        if (sourceNote) summary.source_note = sourceNote;
  ...
  tool(
    ...
    { title: "Delete Load Template", destructiveHint: true },
    async (params) => {
      try {
