 ...
 */
export function toBatchQuoteWidgetData(
  raw: Array<{ row: number; ok: boolean; mode: string; input: Record<string, unknown>; result?: Record<string, unknown>; error?: string }>,
): BatchQuoteWidgetData | null {
  if (!Array.isArray(raw) || raw.length === 0) return null;
  const lanes: BatchLaneRow[] = raw.map((r) => {
    ...
    return {
      row: r.row,
      ok: r.ok === true,
      mode: (["ltl", "ftl", "van", "box-truck"].includes(r.mode) ? r.mode : "ltl") as BatchLaneMode,
...
export function renderBatchQuoteCard(data: BatchQuoteWidgetData): string {
...
export function batchQuoteCardTemplate(): string {
  return buildHtml({ clientScript: OPENAI_CLIENT_JS });
}

export function batchQuoteCardMcpTemplate(): string {
