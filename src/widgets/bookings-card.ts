...
function mapParty(info: unknown): ShipmentParty {
  const i = rec(info);
  const addr = rec(i.address);
  const meta = rec(addr.metadata);
  const contacts = Array.isArray(i.contacts) ? (i.contacts as unknown[]) : [];
  const c = rec(contacts[0]);
  const windows = Array.isArray(i.windows) ? (i.windows as unknown[]) : [];
...
function mapFreight(items: unknown): FreightItem[] {
 ...
 */
export function toBookingsWidgetData(response: unknown): BookingsWidgetData | null {
  ...
  const shipments: ShipmentRow[] = rows.map((raw) => {
    const s = rec(raw);
...
export function renderBookingsCard(data: BookingsWidgetData): string {
  const jsonScript = `<script id="__warp_bk_data" type="application/json">${escapeJsonForScript(
    JSON.stringify(data),
  )}</script>`;
...
export function bookingsCardTemplate(): string {
...
export function bookingsCardMcpTemplate(): string {
